...
describe('BGS → Yes routes to BAS (Beckett Authentication Services), added 2026-09-10', () => {
  ...
  it('Pack-pulled → 1999 - Newer → the BGS w/Auto tiers only, never BAS', () => {
    ...
    // (Cayden 2026-09-12), then four names until BGS retired Priority (2026-09-15); the BAS
    // exclusion is the point of this test and is unchanged.
    expect(names(newer)).toEqual(['BGS Base w/Auto', 'BGS Express w/Auto', 'BGS Standard w/Auto']);
