...
import { CSV_DATA, POLICY, RECOMMENDATIONS, TERMS_OF_USE, PRIVACY_POLICY } from './data';
...
export default function App() {
  const allServices = useMemo(() => parseCSV(CSV_DATA), []);
  ...
  const [activeModal, setActiveModal] = useState<'terms' | 'privacy' | 'cart' | null>(null);
  ...
  const shippingFee = useMemo(() => {
    ...
    const hasGroupA = cart.some(item => /PSA|BGS/i.test(item.service.name));
    const hasGroupB = cart.some(item => /CGC|SGC/i.test(item.service.name));
    return (hasGroupA && hasGroupB) ? 30 : 25;
  }, [cart]);
      ...
      <div className="max-w-4xl mx-auto space-y-12 pt-8 pb-32">
        ...
        <div className="bg-zinc-900/50 rounded-[3rem] p-12 border border-zinc-800 shadow-2xl space-y-10">
          ...
          <div className="space-y-8 pt-6 border-t border-zinc-800">
            <label className="flex items-center gap-8 p-10 bg-zinc-950/50 rounded-3xl cursor-pointer transition-all active:scale-[0.99] border-2 border-transparent has-[:checked]:border-m2m-green group max-w-3xl mx-auto">
              ...
              <span className="text-2xl font-black select-none text-zinc-100 group-hover:text-white transition-colors uppercase italic tracking-tight">
                I acknowledge and agree to the M2M Submission Policy
              </span>
            </label>
        ...
        <div className="flex justify-center gap-12 text-xl font-black text-zinc-500 uppercase tracking-[0.3em] pt-8">
    ...
    return (
      <div className="landscape-container p-8 lg:p-12 flex flex-col overflow-y-auto">
        <div className="flex justify-between items-center mb-12">
          ...
          <button 
            onClick={handleReset}
            className="p-4 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-sm active:rotate-180 transition-transform duration-500"
          >
  ...
  const renderResults = () => (
    <div className="landscape-container p-8 lg:p-12 flex flex-col">
      ...
      <div className="flex justify-between items-center mb-10">
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-1">
            ...
            <div className="inline-flex items-center bg-m2m-green/10 border border-m2m-green/30 px-3 py-1 rounded-full">
              <p className="text-m2m-green font-black uppercase tracking-widest text-[10px]">
      ...
      {/* Main Results Area */}
      <div className="flex-1 overflow-y-auto space-y-8 pb-12 snap-y snap-mandatory scroll-smooth">
            ...
            {remainingServices.map((service, i) => (
              ...
              >
                <div className="p-10 space-y-6 flex-1 overflow-hidden flex flex-col">
                  <div className="flex justify-between items-start gap-12 shrink-0">
                    <div className="space-y-3">
                      <h3 className="text-4xl font-black leading-tight text-white uppercase italic tracking-tight">{service.name}</h3>
                      <div className="flex gap-4">
                        <span className="bg-zinc-950 px-4 py-1.5 rounded-full border border-zinc-800 text-zinc-400 text-xs font-black uppercase tracking-widest">
                          {service.turnaround} Business Days
                        </span>
                        <span className="bg-zinc-950 px-4 py-1.5 rounded-full border border-zinc-800 text-zinc-400 text-xs font-black uppercase tracking-widest">
                          MAX INSURED VALUE IF LOST OR DAMAGED: {service.maxValue}
                        </span>
                      </div>
                    </div>
                    <div className="text-m2m-green font-black text-5xl leading-none">
                      {service.cost}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 flex-1 overflow-hidden">
                    <div className="space-y-4 flex flex-col overflow-hidden">
                      <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500 border-l-4 border-m2m-green pl-4 shrink-0">Service Description</h4>
                    ...
                    <div className="space-y-4 flex flex-col">
                      <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500 border-l-4 border-m2m-green pl-4 shrink-0">ESTIMATED COMPLETION DATE</h4>
                      <div className="bg-zinc-950 p-6 rounded-3xl border border-zinc-800 flex items-center justify-between shrink-0">
                        <div>
                          <p className="text-2xl font-black text-white">{getEstimatedDate(service.turnaround)}</p>
                        </div>
                        <CheckCircle2 className="w-8 h-8 text-m2m-green/20" />
                      </div>
                      ...
                      {service.details && (
                        <div className="p-6 bg-zinc-950/50 rounded-3xl border border-dashed border-zinc-800 flex-1 overflow-y-auto custom-scrollbar">
                          <h4 className="text-[9px] font-black uppercase tracking-[0.3em] text-zinc-500 mb-2">Additional Details</h4>
                ...
                <div className="bg-zinc-950 p-8 flex gap-6 items-center border-t border-zinc-800 shrink-0">
                  ...
                  <button 
                    ...
                    className="flex-1 flex items-center justify-center bg-zinc-800 text-white py-5 rounded-[1.5rem] font-black text-lg uppercase tracking-[0.1em] hover:bg-zinc-700 transition-all shadow-2xl active:scale-95"
                  >
  ...
  const renderHandoff = () => {
    ...
    return (
      <div className="landscape-container bg-zinc-900 flex flex-col items-center justify-center p-8 text-center overflow-hidden relative">
        {/* Store Code Indicator */}
        <div className="absolute top-8 right-8 bg-zinc-950/50 border border-zinc-800 px-4 py-2 rounded-xl">
          <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1">Store Location</p>
          <p className="text-sm font-black text-m2m-green uppercase tracking-tight">{savedStoreCode}</p>
        </div>
  ...
  return (
    <div className="min-h-screen bg-m2m-bg text-zinc-100 selection:bg-m2m-green/20 overflow-hidden">
      {step === 'landing' && <StoreSettings onUpdate={setStoreCode} />}
      ...
      <AnimatePresence>
        {activeModal === 'terms' && (
          <Modal isOpen title="Terms of Use" onClose={() => setActiveModal(null)}>
            <div className="prose prose-sm max-w-none whitespace-pre-wrap text-zinc-300 leading-relaxed">
              {TERMS_OF_USE}
            </div>
          </Modal>
        )}
        ...
        {activeModal === 'cart' && (
          <Modal isOpen title="Checkout Cart" onClose={() => setActiveModal(null)}>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex justify-between items-center">
                          <div className="space-y-1">
                            ...
                            <p className="text-[10px] text-zinc-600 leading-tight uppercase font-black tracking-widest">Flat rate based on service group(s)</p>
                          </div>
                          <span className="text-2xl font-black text-white">
